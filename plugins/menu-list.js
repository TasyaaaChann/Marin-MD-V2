...
let help = Object.values(global.plugins).filter(plugin => !plugin.disabled).map(plugin => {
    ...
    if (teks == '404') {
      const template = generateWAMessageFromContent(m.key.remoteJid, proto.Message.fromObject({
        listMessage: {
            ...
            listType: 1,
            footerText: "*Kalau mau add ke gc ijin owner dulu oke*",
            mtype: 'listMessage',
            sections: [
              {
                "rows": [{
    ...
    await conn.sendHButtonLoc(m.chat,pp, text.trim(), '🅛=limit 🅟=premium', "📍ＴａｓｙａGoblok", instagram, `Kembali Ke List Menu`, `.menu`, m)
} catch (e) {
